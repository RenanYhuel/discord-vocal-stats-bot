...
import { recordCompletedSession, recordDeafSession, checkRankOvertake } from "../managers/voiceManager";
...
export default {
  ...
  async execute(oldState: VoiceState, newState: VoiceState, client: Client): Promise<void> {
    const userId = newState.id;
    const username = newState.member ? newState.member.user.username : "Inconnu";
    ...
    else if (oldState.channelId && !newState.channelId && oldState.channel) {
      try {
        db.insert(voiceEvents).values({
        ...
        if (current) {
          if (current.isDeaf && current.deafenedAt) {
            recordDeafSession(userId, username, oldState.channel.name, current.deafenedAt, timestamp);
          }
          recordCompletedSession(client, userId, username, oldState.channel.name, current.joinedAt, timestamp);
        }
        db.delete(voiceCurrent).where(eq(voiceCurrent.userId, userId)).run();
      } catch (err) {
        // Ignorer
      }
    }
    else if (oldState.channelId && newState.channelId && oldState.channelId !== newState.channelId && oldState.channel && newState.channel) {
      try {
        db.insert(voiceEvents).values({
          messageId: `live_leave_${userId}_${Date.now()}`,
          userName: username,
    ...
    if (oldState.channelId && newState.channelId && oldState.channelId === newState.channelId && newState.channel) {
      ...
      if (!wasDeaf && isDeaf) {
        ...
        if (current && current.deafenedAt) {
          recordDeafSession(userId, username, newState.channel.name, current.deafenedAt, timestamp);
        }
        db.update(voiceCurrent)
    ...
    if (newState.channelId) {
      ...
      if (activeUser) {
        const currentDurationSec = Math.floor((new Date().getTime() - new Date(activeUser.joinedAt).getTime()) / 1000);
        if (currentDurationSec > 60) {
          await checkRankOvertake(client, userId, username, currentDurationSec);
        }
      }
    }
  }
