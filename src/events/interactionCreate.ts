...
export default {
    ...
    async execute(
        ...
        try {
            logger.info(
                `[COMMAND] Executing command: /${interaction.commandName} by ${interaction.user.tag}`,
            );
            await command.execute(interaction, client);
        } catch (error) {
            logger.error(
                `Erreur d'exécution de la commande /${interaction.commandName}`,
                error,
            );
