...
export default {
  ...
  async execute(client: Client): Promise<void> {
    ...
    try {
      ...
      logger.error("Erreur critique lors de l'initialisation au démarrage", err);
    }
  }
};
...
async function catchUp(client: Client): Promise<void> {
  ...
  const channel = await client.channels.fetch(config.carlLogChannelId).catch(() => null) as TextChannel | null;
  if (!channel) {
    return;
  }

  const lastMessageState = db.select().from(state).where(eq(state.key, "last_message_id")).get();
  ...
  let hasMore = true;
  while (hasMore) {
    const fetchedMessages = await channel.messages.fetch({ limit: 100, after: lastMessageId }).catch(() => null);
    if (!fetchedMessages || fetchedMessages.size === 0) {
      hasMore = false;
      break;
    }
    const sortedMessages: Message[] = Array.from(fetchedMessages.values()).sort((a, b) => a.createdTimestamp - b.createdTimestamp);
    ...
    for (const msg of sortedMessages) {
        ...
        msg.embeds.forEach((embed) => {
          ...
          if (type) {
            ...
            if (embed.footer && embed.footer.text) {
              const match = embed.footer.text.match(/ID:\s*(\d+)/i);
              if (match) {
                userId = match[1];
              }
            }
            ...
            if (userId && userMatch && chanMatch) {
              ...
              const unixSec = Math.floor(new Date(timestampStr).getTime() / 5000);
              ...
              try {
                ...
                if (type === "voice_leave") {
                  const lastJoin = db.select({
                    timestamp: voiceEvents.timestamp
                  })
                  .from(voiceEvents)
                  .where(and(eq(voiceEvents.userId, userId), eq(voiceEvents.type, "voice_join"), lte(voiceEvents.timestamp, timestampStr)))
                  .orderBy(desc(voiceEvents.timestamp))
                  .limit(1)
                  .get() as DBLastJoinQuery | undefined;

                  if (lastJoin) {
                    recordCompletedSession(client, userId, username, channelName, lastJoin.timestamp, timestampStr);
                  }
                }
              } catch (err) {
                // Déjà dédoublé
      ...
      db.insert(state).values({ key: "last_message_id", value: lastMessageId }).onConflictDoUpdate({
        target: state.key,
...
async function syncActiveVoiceStates(client: Client): Promise<void> {
  ...
  const realActiveUsers = new Map<string, { username: string; channelName: string; channelId: string; isDeaf: number }>();
  ...
  }).from(voiceCurrent).all() as DBActiveUserQuery[];
  ...
  dbActiveUsers.forEach(dbUser => {
    if (!realActiveUsers.has(dbUser.user_id)) {
      try {
        ...
        recordCompletedSession(client, dbUser.user_id, dbUser.username, dbUser.channel_name, dbUser.joined_at, timestamp);
      } catch (err) {
        // Ignorer
      }
      db.delete(voiceCurrent).where(eq(voiceCurrent.userId, dbUser.user_id)).run();
    }
  });
