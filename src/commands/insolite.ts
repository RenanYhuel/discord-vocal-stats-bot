...
export default {
    data: new SlashCommandBuilder()
        .setName("insolite")
        .setDescription(
            "Classements amusants et statistiques insolites du serveur",
        )
        .addStringOption((option) =>
            option
                ...
                .addChoices(
                    ...
                    {
                        name: "Les Solitaires (Temps passé seul)",
                        value: "alone",
                    },
                    {
                        name: "Les plus longues sessions d'affilée",
                        value: "marathon",
                    },
                ),
        ),
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        if (type === "night") {
            ...
            embed
                .setTitle("Les insomniaques de nuit (00h-06h)")
            ...
            embed
                .setTitle("Le Roi de la Connexion / Déconnexion")
                .setDescription(text || "Aucune donnée");
        } else if (type === "alone") {
            const data = db
                .select({
          ...
          SELECT user_id, user_name, SUM(alone_sec) as total_sec FROM alone_durations GROUP BY user_id
            ...
            embed
                .setTitle(
                    "Les Solitaires (Sessions 100% seuls du début à la fin)",
                )
                .setDescription(text || "Aucune donnée");
            ...
            embed
                .setTitle(
                    "Les plus longues sessions individuelles ininterrompues",
                )
                .setDescription(text || "Aucune donnée");
        }
