...
export default {
    data: new SlashCommandBuilder()
        .setName("badges")
        .setDescription(
            "Découvre les rôles virtuels et trophées décernés aux membres",
        ),
    async execute(interaction: CommandInteraction): Promise<void> {
        ...
        try {
            const topNight = db
                ...
                .from(voiceSessions)
                .where(
                    sql`strftime('%H', datetime(${voiceSessions.joinTime}, '+2 hours')) >= '00' AND strftime('%H', datetime(${voiceSessions.joinTime}, '+2 hours')) < '06'`,
                )
            ...
            const topMorning = db
                ...
                .from(voiceSessions)
                .where(
                    sql`strftime('%H', datetime(${voiceSessions.joinTime}, '+2 hours')) >= '06' AND strftime('%H', datetime(${voiceSessions.joinTime}, '+2 hours')) < '12'`,
                )
                .groupBy(voiceSessions.userId)
                .orderBy(sql`SUM(${voiceSessions.durationSec}) DESC`)
                .limit(1)
            ...
            const topWeekend = db
                ...
                .from(voiceSessions)
                .where(
                    sql`strftime('%w', ${voiceSessions.joinTime}) IN ('0', '6')`,
                )
                .groupBy(voiceSessions.userId)
                .orderBy(sql`SUM(${voiceSessions.durationSec}) DESC`)
                .limit(1)
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor("#FEE75C")
                .setDescription(
                    "Voici les champions actuels détenant les badges du serveur :",
                )
                .addFields([
                    {
                        name: "Le Hibou Suprême (Activité de nuit, 00h-06h)",
                        value: topNight
                            ? `<@${topNight.userId}>\n${formatDurationDetailed(topNight.nightSec)}`
                            : "Aucun",
                    },
                    {
                        name: "Le Marathonien (Plus longue session d'affilée)",
                        value: topMarathon
                            ? `<@${topMarathon.userId}>\n${formatDurationDetailed(topMarathon.maxSec)}`
                            : "Aucun",
                    },
                    {
                        name: "Le Coq du Matin (Activité matinale, 06h-12h)",
                        value: topMorning
                            ? `<@${topMorning.userId}>\n${formatDurationDetailed(topMorning.morningSec)}`
                            : "Aucun",
                    },
                    {
                        name: "Le Caméléon (A exploré le plus de salons différents)",
                        value: topCameleon
                            ? `<@${topCameleon.userId}> avec \`${topCameleon.uniqueChans}\` salons visités.`
                            : "Aucun",
                    },
                    {
                        name: "Le Loup Solitaire (Plus grand nombre de sessions vocales)",
                        value: topLone
                            ? `<@${topLone.userId}> avec \`${topLone.sessionsCount}\` sessions.`
                            : "Aucun",
                    },
                    {
                        name: "Le Pilier de Comptoir (Plus de sessions de moins de 2 minutes)",
                        value: topLizard
                            ? `<@${topLizard.userId}> avec \`${topLizard.shortCount}\` apparitions éclairs.`
                            : "Aucun",
                    },
                    {
                        name: "Le Tryharder (Plus grand nombre de sessions > 4 heures)",
                        value: topTryhard
                            ? `<@${topTryhard.userId}> avec \`${topTryhard.longCount}\` sessions.`
                            : "Aucun",
                    },
                    {
                        name: "Le Guerrier du Weekend (Le plus actif le samedi/dimanche)",
                        value: topWeekend
                            ? `<@${topWeekend.userId}>\n${formatDurationDetailed(topWeekend.weekendSec)}`
                            : "Aucun",
                    },
                    {
                        name: "L'Instable (Le plus d'allers-retours)",
                        value: topJumper
                            ? `<@${topJumper.userId}> avec \`${topJumper.totalEvents}\` sauts de connexion.`
                            : "Aucun",
                    },
                    {
                        name: "Le Chaperon (Plus de temps dans des sessions de discussion de groupe)",
                        value: topHost
