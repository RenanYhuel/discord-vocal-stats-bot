    ...
    async execute(interaction: CommandInteraction): Promise<void> {
      ...
      `,
            ...
            allSessions.forEach((s) => {
                ...
                chanIntervals[s.channelName].push({
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields([
                    ...
                    {
                        name: "Activité continue record d'un salon (tous membres confondus)",
                        value: `\`#${maxContinuousChan}\` :\n${formatDurationDetailed(maxContinuousSec)}`,
                    },
                    {
                        name: "Journée historique la plus active",
                        value: topDay
                            ? `\`${new Date(topDay.date).toLocaleDateString("fr-FR")}\` avec un cumul de :\n${formatDurationDetailed(topDay.dailySec)}`
                            : "Aucun",
                    },
                    {
                        name: "Le Salon Mythique (Salon le plus fréquenté)",
                        value: topChan
                            ? `\`#${topChan.channelName}\` avec un total de :\n${formatDurationDetailed(topChan.totalSec)}`
                            : "Aucun",
                    },
                    {
                        name: "Le Mois d'Or",
                        value: topMonth
                            ? `\`${topMonth.month}\` avec un total de :\n${formatDurationDetailed(topMonth.totalSec)}`
                            : "Aucun",
                    },
                    {
                        name: "Jour de la semaine le plus actif historiquement",
                        value: weekDayQuery
                            ? `\`${weekDayQuery.dayOfWeek}\` avec un cumul de :\n${formatDurationDetailed(weekDayQuery.totalSec)}`
                            : "Aucun",
                    },
                    {
                        name: "La Saison d'Or (Saison la plus active)",
                        value: seasonQuery
                            ? `${seasonQuery.season} avec un cumul de :\n${formatDurationDetailed(seasonQuery.totalSec)}`
                            : "Aucun",
                    },
                    {
                        name: "La Session Éclair (Record du miss-click le plus rapide)",
                        value: minSession
                            ? `<@${minSession.userId}> avec une session de seulement \`${minSession.durationSec}s\` (le ${new Date(minSession.timestamp).toLocaleDateString("fr-FR")})`
                            : "Aucun",
                    },
                ])
                .setTimestamp();
