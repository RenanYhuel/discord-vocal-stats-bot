...
export default {
  data: new SlashCommandBuilder()
    .setName("stats")
    .setDescription("Affiche tes statistiques de présence vocale détaillées (avec gestion sourdine)")
    .addUserOption(option => option.setName("cible").setDescription("Le membre à analyser (optionnel)")),
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (!statsQuery || statsQuery.totalSec === null) {
      await interaction.editReply(`Aucune statistique enregistrée pour <@${target.id}>.`);
      return;
    }

    const rankQuery = db.select({
    ...
    const badges: string[] = [];
    if (nightSec > statsQuery.totalSec * 0.40) {
    ...
    const displayRecord = rank <= 10 ? formatDurationDetailed(statsQuery.maxSessionSec) : formatDurationStandard(statsQuery.maxSessionSec);
