...
export async function checkRankOvertake(
    ...
    try {
        ...
        logger.error(
            "Erreur lors de la vérification du dépassement de classement",
            err,
        );
    }
}
...
export function recordCompletedSession(
    ...
    try {
        ...
        logger.info(
            `[STATS] Session enregistrée pour ${username} (${channelName}) : ${durationSec}s (Actif: ${activeSec}s, Sourdine: ${deafSec}s)`,
        );
        ...
        logger.error(
            `Erreur lors de l'enregistrement de la session pour ${username}`,
            err,
        );
    }
}

export function recordDeafSession(
    userId: string,
    username: string,
    channelName: string,
    startTimeStr: string,
    ...
    try {
        ...
        logger.info(
            `[DEAF] Sourdine casque enregistrée pour ${username} : ${durationSec}s`,
        );
    } catch (err) {
        logger.error(
            `Erreur lors de l'enregistrement de sourdine pour ${username}`,
            err,
        );
    }
}
