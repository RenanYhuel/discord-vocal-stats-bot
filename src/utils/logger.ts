...
function writeLog(
    ...
    const cleanObj =
        obj instanceof Error ? { message: obj.message, stack: obj.stack } : obj;
    ...
    try {
        ...
        if (err instanceof Error) {
            console.error(
                "Impossible d'écrire le log système en DB : ",
                err.message,
            );
        }
    }
...
export default {
    info: (message: string, obj: unknown = null) =>
        writeLog("info", message, obj),
    warn: (message: string, obj: unknown = null) =>
        writeLog("warn", message, obj),
    error: (message: string, obj: unknown = null) =>
