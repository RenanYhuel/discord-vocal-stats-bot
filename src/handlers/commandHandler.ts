...
export function loadCommands(client: any): void {
  ...
  for (const file of commandFiles) {
    ...
    if (cmdModule && "data" in cmdModule && "execute" in cmdModule) {
      ...
      logger.warn(`La commande à ${filePath} manque de propriétés requises.`);
    }
  }
  ...
  (async () => {
    try {
      logger.info("Enregistrement des commandes Slash auprès de l'API Discord...");
      await rest.put(
        Routes.applicationGuildCommands(config.clientId, config.guildId),
        { body: commandsData }
      );
      logger.info("Commandes Slash enregistrées avec succès auprès de Discord !");
    } catch (error) {
